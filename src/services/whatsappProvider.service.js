...
async function sendWhatsApp(message) {
  ...
  if (consoleDeliveryEnabled('WHATSAPP')) return logConsoleDelivery('WHATSAPP', message, { source: 'whatsapp-provider', provider: provider || 'console' });
  if (!provider || provider === 'disabled') return { status: 'SKIPPED', error: 'WhatsApp provider is not configured' };
  if (process.env.NODE_ENV === 'test') return { status: 'SKIPPED', error: 'WhatsApp provider is not configured' };
  if (provider === 'console') return logConsoleDelivery('WHATSAPP', message, { source: 'whatsapp-provider', provider: 'console' });
